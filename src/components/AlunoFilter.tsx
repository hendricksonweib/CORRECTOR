...
export const AlunoFilter = ({ onFilter }: AlunoFilterProps) => {
  ...
  useEffect(() => {
    const fetchEscolas = async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/escolas?page=1&limit=200`);
      const data = await res.json();
      setEscolas(Array.isArray(data.data) ? data.data : data);
    };

    const fetchProvas = async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/provas`);
      const data = await res.json();
  ...
  useEffect(() => {
    ...
    const fetchTurmas = async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/turmas?escola_id=${escolaId}`);
      const data = await res.json();
